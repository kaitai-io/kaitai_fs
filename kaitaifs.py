...
from fs.heroes_of_might_and_magic_agg_fs import HeroesOfMightAndMagicAggFS
...
def parse_args():
    parser = argparse.ArgumentParser(description='Mount a filesystem based on Kaitai Struct spec.')
    parser.add_argument(
        'fstype', metavar='TYPE', choices=FILESYSTEMS.keys(),
        help='filesystem type'
    )
    parser.add_argument(
        'image_file', metavar='IMAGE_FILE', type=str,
        help='source image file (or block device)'
    )
    parser.add_argument(
        'mount_point', metavar='MOUNT_POINT', type=str,
        help='mount point'
    )
